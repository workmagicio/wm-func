...
REGION="us-east1"
SERVICE_NAME="fairing-v2-first-sync-service" # 服务名称，可自定义
...
(cd "${PROJECT_ROOT}" && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-w -s" -v -o "${SCRIPT_DIR}/server" ./cmd/fairing_v2_first_sync)
