...
REGION="us-east1"
JOB_NAME="meta-ad-metrics-new-tenant"
...
(cd "${PROJECT_ROOT}" && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-w -s" -v -o "${SCRIPT_DIR}/server" ./cmd/meta-ad-metrics)
